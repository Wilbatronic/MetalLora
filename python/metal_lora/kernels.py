...
def lora_forward_metal(
    ...
    # SIMD kernel is currently broken (low thread utilization), always use basic kernel
    use_simd = False
    ...
    if use_simd:
        ...
        # Grid: (ceil(out_features/256), seq_len, batch) - 256 threads per threadgroup
        num_tg_x = (out_features + 255) // 256
        grid = (num_tg_x * 256, seq_len_actual, batch_size_actual)  # Total threads
        threadgroup = (256, 1, 1)
        template = [("T", x.dtype)]
